...
console.log("------------------------");
	...
	const renderPokemon = ({id, name, likes, img}) => {
		...
		// [pokeCard.id, pokeImg.src, pokeName.textContent, likesNum.textContent] = [`poke-${pokemon.id}`, pokemon.img, pokemon.name, pokemon.likes]
		pokeCard.id = id;
		...
		likeBttn.addEventListener("click", () => increaseLike(likes, id));
	...
	const renderComment = ({name, content}) => {
	...
	const deletePoke = (pokemon, pokeCard) => {
...
// 🚧 Break Out Activity 2: Destructuring Objects & Arrays
	...
	// directly using object destructuring.
		...
		// 	Example: 
		// const {id, name, likes, img} = pokeCard
		// 	const renderPokemon = (pokeCard) => {
		// 		pokeCard.id = `poke-${id}`;
		// 		pokeCard.name = pokemon.name;
		// 		pokeCard.likes = pokemon.likes;
		// 		pokeCard.src = pokemon.img;
		// 	}
